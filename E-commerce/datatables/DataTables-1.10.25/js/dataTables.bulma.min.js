...
$jscomp.polyfillIsolated = function(a, b, c, e) {
    ...
    for (var f = 0; f < d.length - 1; f++) { var k = d[f]; if (!(k in e)) return;
        e = e[k] }
    d = d[d.length - 1];
...
(function(a) { "function" === typeof define && define.amd ? define(["jquery", "datatables.net"], function(b) { return a(b, window, document) }) : "object" === typeof exports ? module.exports = function(b, c) { b || (b = window);
    ...
    a.extend(!0, d.defaults, {
        dom: "<'columns is-gapless is-multiline'<'column is-one-half'l><'column is-one-half'f><'column is-full'tr><'column is-one-half'i><'column is-one-half'p>>",
        renderer: "bulma"
    });
    a.extend(d.ext.classes, { sWrapper: "dataTables_wrapper dt-bulma", sFilterInput: "input", sLengthSelect: "custom-select custom-select-sm form-control form-control-sm", sProcessing: "dataTables_processing card" });
    d.ext.renderer.pageButton.bulma = function(f, k, D, E, m, v) {
        var w = new d.Api(f),
            p = f.oLanguage.oPaginate,
            F = f.oLanguage.oAria.paginate || {},
            h, l, x = 0,
            A = function(t, y) {
                var z, G = function(q) { q.preventDefault();
                    a(q.currentTarget).hasClass("disabled") || w.page() == q.data.action || w.page(q.data.action).draw("page") };
                var u = 0;
                    ...
                    if (Array.isArray(g)) A(t, g);
                        ...
                        switch (g) {
                            ...
                            case "last":
                                ...
                                n = m >= v - 1; break;
                            default:
                                h = g + 1, l = m === g ? "is-current" : "" }
                        h && (r = a("<li>", { id: 0 === D && "string" === typeof g ? f.sTableId + "_" + g : null }).append(a("<" + r + ">", {
                            href: "#",
                            ...
                            disabled: n
                        }).html(h)).appendTo(t), f.oApi._fnBindAction(r, { action: g }, G), x++)
                    }
                }
            };
        try { var B = a(k).find(c.activeElement).data("dt-idx") } catch (t) {}
        var C = a('<nav class="pagination" role="navigation" aria-label="pagination"><ul class="pagination-list"></ul></nav>');
        a(k).empty().append(C);
        A(C.find("ul"), E);
        B !== e && a(k).find("[data-dt-idx=" + B + "]").trigger("focus")
    };
    a(c).on("init.dt", function(f, k) {
        "dt" === f.namespace && (f = new a.fn.dataTable.Api(k),
            a("div.dataTables_length select", f.table().container()).wrap('<div class="select">'))
    });
    return d
});